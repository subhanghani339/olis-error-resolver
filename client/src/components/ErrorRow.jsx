    ...
    <div className="border-t-2 border-gray-200 p-4">
      <div className="grid grid-cols-12 gap-4 items-start">
        ...
        <div className="col-span-2">
          ...
          <div className="text-xs text-gray-600 mt-1">{error.dateSubmitted}</div>
        </div>
        ...
        <div className="col-span-2">
          ...
          {error.status === 'resolved' ? (
            <>
              <div className="text-xs text-gray-900">{error.resolvedBy?.split(' - ')[1] || '—'}</div>
              <div className="text-xs text-gray-600 mt-0.5">{error.resolvedBy?.split(' - ')[0] || '—'}</div>
            </>
          ) : (
        ...
        <div className="col-span-2">
          ...
          <div className="flex flex-wrap gap-1">
            {error.hl7Segments.map((segment, index) => (
              <span
                key={index}
                className="inline-block bg-blue-100 text-blue-900 text-xs font-medium px-2 py-0.5 rounded"
              >
                {segment}
              </span>
            ))}
        ...
        <div className="col-span-3">
          {error.status === 'resolved' ? (
            <>
              <div className="text-xs text-gray-700 mb-2">{error.comment || '—'}</div>
